    ...
    vector<int> dijkstra(int V, vector<vector<int>> &edges, int src) {
        ...
        while(!pq.empty()){
            ...
            for(int i=0; i<adjList[node].size(); i++){
                ...
                if(dist[connected_node] > distance + wt){
                    dist[connected_node] = distance + wt;
...
/*
Time complexity: O(ElogV), where E is the number of edges present in the graph and V is the number of vertices present in the graph.
